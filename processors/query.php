    ...
    private function process () : bool {
        ...
        foreach ($words as $word){
            ...
            if(!$this->record_exists_in_table($this->words_table_name , 'word' , $word)) {
                ...
                $this->executeSQL("UPDATE {$this->words_table_name} SET occurrence = occurrence + 1, last_search = '{$now}' WHERE word = '{$word}'");
            }
        }
        ...
        return $this->record_exists_in_table($this->queries_table_name , "query" , $this->query) ? $this->executeSQL("UPDATE $this->queries_table_name SET occurrence = occurrence + 1, last_search = '{$now}' WHERE query = '{$this->query}'") : $this->insert_into_table($this->queries_table_name , ["query" => $this->query , "occurrence" => 1]);
