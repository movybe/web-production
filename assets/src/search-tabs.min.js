    ...
    render() {
        ...
        const tabList =  locale.map((local , index) => {
            ...
            return (
                ...
                <li id = {local.shortName + "-tab"} onClick={
                    ...
                    key = {local.name} className="tab website-list-tabs"><a href= {"#" + local.shortName} id = {local.shortName + "-tab-link"} className={"tab-links " + active}><img alt = {local.shortName} title = {local.shortName} data-src={defaults.imageDirectory + local.shortName +'.png'} className="responsive-img tab-icons lazyload" /></a></li>
        ...
        let tabContainers = locale.map((local , pos)  => {
            ...
            let template = (local.ads.length) ? local.ads.map((ad, index) => {
                ...
                lastUpdatedTimestamp = local.lastUpdatedTimestamp? <h6><span className='red-text'>Last update : </span> <span className ='stored-timestamp' data-datetime={local.lastUpdatedTimestamp || '2019-11-05 00:42:08'}></span></h6>: null;
