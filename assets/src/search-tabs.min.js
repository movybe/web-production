...
class  LocalSearchTab extends React.Component{
    ...
    defaultActions = () => {
        ...
        timeago.render(document.querySelectorAll('.stored-timestamp'));
    ...
    componentDidUpdate() {
        ...
        let currentPrice, max , priceString , pricesArrayLength  , allPricesArray , totalPrice , x ,sum, price , priceList , adsLength , isOddAdLength , average , priceListLengthDividedBy4 , priceListLengthDividedBy2, median , middleSum ,sortAdInAscendingOrder , medianPlusMax;
        ...
        locale.forEach(local => {
            ...
            local.ads.forEach((ad) => {
                ...
                if(currentPrice.indexOf('-') < 0 && currentPrice.indexOf(' ') < 0) {

                    price = parseInt(currentPrice.replace(/\D/g, ''));
                    ...
                    pricesArrayLength = 0;
                    allPricesArray = currentPrice.split(' ') || currentPrice.split('-');

                    totalPrice = 0;

                    allPricesArray.forEach(price => {
                        ...
                        if(priceString) {
                            ++pricesArrayLength;
                            totalPrice += parseInt(priceString);
                        }

                    });

                    price = parseInt(totalPrice / pricesArrayLength);
                }
            ...
            local.max = numeral(max /*local.ads[local.ads.length -1].price */).format('0.0a');
    ...
    handleFacebookShareButtonClick = e => {

        this.props.switchWebsite({...this.props , sharedFacebookPage: true});
    ...
    render() {
        ...
        const tabList =  locale.map((local , index) => {
            ...
            return (
                ...
                <li id = {local.shortName + "-tab"} onClick={
                    ...
                    key = {local.name} className="tab website-list-tabs"><a href= {"#" + local.shortName} id = {local.shortName + "-tab-link"} className={"tab-links " + active}><img alt = {defaults.capitalize(local.shortName)} title = {defaults.capitalize(local.shortName)} data-src={defaults.imageDirectory + local.shortName +'.png'} className="responsive-img tab-icons lazyload" /></a></li>
        ...
        let loadMoreButton;
        let facebookShareFooter = null;
        facebookShareFooter = this.props.sharedFacebookPage ? null : <footer className="white darken-2 " id="site-footer">
            ...
            <div className="footer-copyright">
                <div className="container">
                    <h6 className="blue-text left" id="share-on-faceboox-text">Share on Facebook</h6>


                    <h6 className="right">
                        <a className="btn btn-block blue facebook-share-button text-lighten-3 right" onClick={this.handleFacebookShareButtonClick} target="_blank" href={"https://www.facebook.com/sharer/sharer.php?u=" + window.location.hostname}>Share</a>
        ...
        let tabContainers = locale.map((local , pos)  => {
            ...
            let template = (local.ads.length) ? local.ads.map((ad, index) => {
                ...
                lastUpdatedTimestamp = local.lastUpdatedTimestamp? <h6><span className='red-text'>Updated : </span> <span className ='stored-timestamp' data-datetime={local.lastUpdatedTimestamp || '2019-11-05 00:42:08'}></span></h6>: null;
                showAdImage = (!this.props.settings.showImages && !ad.showAdImage) ? <a href="#"  className="show-ad-image image-download-link search-result-images blue-text" onClick={e => {e.preventDefault(); this.toggleImageView(e , local , pos ,   ad , index)}}>View</a> : null;
                showAdImage = (showAdImage === null && !this.props.settings.showImages && ad.showAdImage) ? <a href="#"  className="show-ad-image image-download-link search-result-images blue-text" onClick={e => {e.preventDefault(); this.toggleImageView(e , local , pos ,   ad , index , false)}}>Hide</a>: showAdImage;
                ...
                return (
                    ...
                    <div className="search-result" key = {Math.random()}>
                        ...
                        <h3 className="search-result-title-header" title = {ad.long_title || null}>
                            <a target="_blank" data-ad-id = {isValidSponsoredAd ? ad.ad_id : null}
                               onClick={isValidSponsoredAd ? this.handleSponsoredAdClicked : null}
                               className="search-result-title-link" href={ad.link}>
                                {ad.title}
                            </a>
                        </h3>
                        ...
                        <span className="search-result-link-description" title={ad.long_description || null}>
{ad.description }
</span>
                        {showImages}
            ...
            return (
                ...
                <div id={local.shortName} className="col s12 gallery" key={local.name}>
                    ...
                    <p className='flow-text' style={{color : local.textColor,  'textTransform' : 'capitalize'}}>{local.name} {averagePrice}</p>
