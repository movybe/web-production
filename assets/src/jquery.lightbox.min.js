function closeLightBox(e){const id=$(e).attr('data-id');$('.lightbox').fadeOut('fast');$('body').removeClass('blurred');$(document).off('keydown');return!1}(function($){'use strict';$.fn.lightbox=function(options){var opts={margin:50,nav:!0,blur:!0,minSize:0};var plugin={items:[],lightbox:null,image:null,current:null,locked:!1,caption:null,init:function(items){plugin.items=items;var id='lightbox-'+Math.floor((Math.random()*100000)+1);$('body').append('<div id="'+id+'" class="lightbox" style="display:none;">'+'<span class="lightbox__close lightbox__button" onclick="return closeLightBox(this)" data-id = "'+id+'"></span>'+'<span  class="lightbox__nav lightbox__nav--prev lightbox__button"></span>'+'<span  class="lightbox__nav lightbox__nav--next lightbox__button"></span>'+'<div href="#" class="lightbox__caption"><p></p></div>'+'</div>');plugin.lightbox=$("#"+id);plugin.caption=$('.lightbox__caption',plugin.lightbox);if(plugin.items.length>1&&opts.nav){$('.lightbox__nav',plugin.lightbox).show()}else{$('.lightbox__nav',plugin.lightbox).hide()}
        plugin.bindEvents()},loadImage:function(){if(opts.blur){$("body").addClass("blurred")}
        $("img",plugin.lightbox).remove();plugin.lightbox.fadeIn('fast').append('<span class="lightbox__loading"></span>');var img=$('<img src="'+$(plugin.current).attr('href')+'" draggable="false">');$(img).on('load',function(){$('.lightbox__loading').remove();plugin.lightbox.append(img);plugin.image=$("img",plugin.lightbox).hide();plugin.resizeImage();plugin.setCaption()})},setCaption:function(){var caption=$(plugin.current).data('caption');if(!!caption&&caption.length>0){plugin.caption.fadeIn();$('p',plugin.caption).html(caption)}else{plugin.caption.hide()}},resizeImage:function(){var ratio,wHeight,wWidth,iHeight,iWidth;wHeight=$(window).height()-opts.margin;wWidth=$(window).outerWidth(!0)-opts.margin;plugin.image.width('').height('');iHeight=plugin.image.height();iWidth=plugin.image.width();if(iWidth>wWidth){ratio=wWidth/iWidth;iWidth=wWidth;iHeight=Math.round(iHeight*ratio)}
        ...
        var self=$(this)[0];e.preventDefault();plugin.current=self;plugin.loadImage();$(document).on('keydown',function(e){if(e.keyCode===27){plugin.close()}
            ...
            if(e.keyCode===37){plugin.previous()}})});plugin.lightbox.on('click',function(e){if(this===e.target){plugin.close()}});$(plugin.lightbox).on('click','.lightbox__nav--prev',function(){plugin.previous();return!1});$(plugin.lightbox).on('click','.lightbox__nav--next',function(){plugin.next();return!1});$(window).resize(function(){if(!plugin.image){return}
